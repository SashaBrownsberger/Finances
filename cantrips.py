...
def sigClipMean(list_for_mean, sig_clip = np.inf):
    ...
    while still_clipping:
        current_mean = np.nanmean(clipped_list)
        current_std = np.nanstd(clipped_list)
...
def plotBarGraphStack(xs, list_of_stacks, list_of_colors, ax = None, figsize = (10, 15), show = 0):
    if ax == None:
    ...
    for j in range(n_stacks):
        ...
        positive_stack  = np.sum([elem for elem in stack if elem >= 0.0])
        negative_stack  = np.sum([elem for elem in stack if elem < 0.0])
        for i in range(len(stack)):
            elem = stack[i]
            color = colors[i]
            if elem < 0.0:
                bar = ax.bar(x, negative_stack, color = color)[0]
                negative_stack = negative_stack - elem
            else:
                bar = ax.bar(x, positive_stack, color = color)[0]
                positive_stack = positive_stack - elem
            bars = bars + [bar]
...
def readInRowsToList(file_name, file_dir = '', n_ignore = 0, delimiter = ' ', convert_to_float = 0, convert_to_int = 0, verbose = 1):
    ...
    """
    lines = readInFileLineByLine(file_name, file_dir = file_dir, n_ignore = n_ignore, verbose = verbose)
    lines = [[elem for elem in line.split(delimiter) if elem != ''] for line in lines ]
    rows = lines
    if convert_to_float:
